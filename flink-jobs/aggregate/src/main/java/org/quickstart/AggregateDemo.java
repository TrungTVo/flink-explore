...
/**
 * Flink streaming job that computes the average order price per electronic product ID
 * using a 10-second tumbling event-time window, combining an incremental
 * {@link AvgWindowFunction} with a {@link AvgProcessWindowFunction} for window metadata.
 */
public class AggregateDemo {
    ...
    private static class AvgWindowFunction implements AggregateFunction<ElectronicOrder, Tuple2<Double, Integer>, Double> {
        ...
        /** Returns initial zeroed (sum=0.0, count=0) accumulator. */
        @Override
        ...
        /**
         * Computes the final average from the accumulated sum and count.
         *
         * @return sum / count
         */
        @Override
         ...
         * This is used when Flink needs to combine results from parallel sub-tasks or when windows are merged.
         * Usually, with non-merging windows like tumbling, this method won't be called, but it's required to implement the interface.
         * For session windows or other merging window types, this method would be essential to correctly combine partial results.
         *
    ...
    /**
     * Receives the single pre-aggregated average from {@link AvgWindowFunction}, logs the window
    ...
    private static class AvgProcessWindowFunction extends ProcessWindowFunction<Double, Double, String, TimeWindow> {
        ...
        /**
         * Logs window metadata (key, start/end time, batch size) and forwards the average price.
         *
         * @param key         the electronic product ID for this window
         * @param ctx         provides access to window timing metadata
         * @param runningAccs single-element iterable containing the pre-aggregated average
         * @param out         collector for emitting the average downstream
         */
        @Override
        public void process(
                String key,
